...
function buildTemplateWeek4WF({ client, copy, images, footerData, isHeroGenerated = false,
  ...
  const entryHtml = (line, k) => {
    ...
    return `<div style="margin-top:${k ? 16 : 10}px;">
  ...
  /* Each block opens with the three block photos as upright prints fanned to
     ...
     block 2 with Sub 2, block 3 with Sub 3. Front print 260x340, a clear
     portrait; the two behind step 40px right and 10px down and lose 20px of
     height each, so the stage is 340x340. Sized in percentages of that stage
     so it scales on a phone.
     Once baked, the whole fan is one PNG. Same numbers in week4wfStackHtml. */
  const STAGE_W = 340, STAGE_H = 340, PRINT_W = 260, STEP_X = 40, STEP_Y = 10, STEP_H = 20
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   three points each with a baked line icon, the cancellation policy in plain
   English, then the one CTA. Everything below the hero follows
   DESIGN_SYSTEM.md; the headline is hero copy and exempt.
   ══════════════════════════════════════════════════════════════════════════ */
function buildTemplateWeek6WF({ client, copy, images, footerData, isHeroGenerated = false,
  heroScale=1, heroX=0, heroY=0,
  textSize=44, textTop=34, textLeft=52,
  logoColor='original', logoTop=64, logoRight=200, logoSize=44,
  img1Scale=1, img1X=0, img1Y=0,
  ...
  const closing    = (copy.closingLine || '').replace(/\n/g, '<br>')
  const policy     = (copy.bodyBlock2  || '').replace(/\n/g, '<br>')
  const footerLine = (copy.footerLine  || '').replace(/\n/g, '<br>')
  const subhead    = (copy.sectionSubhead || copy.subhead || '').replace(/\n/g, '<br>')
  const logoUrl = client?.logoUrl || ''
  ...
  /* The intro arrives as one or two paragraphs; a blank line is a break. */
  const introParas = intro ? intro.split(/\n\s*\n/).map(x => x.trim()).filter(Boolean).map(x => x.replace(/\n/g, '<br>')) : []

  const points = Array.isArray(copy.points) ? copy.points.filter(p => p && (p.title || p.text)).slice(0, 5) : []
...
<table class="w6wf-outer" cellpadding="0" cellspacing="0" bgcolor="${pageBg}" style="width:100%;max-width:600px;margin:0 auto;background-color:${pageBg};border-collapse:collapse;border-radius:20px;overflow:hidden;">
<tr><td style="background-color:${pageBg};">

  <!-- HERO — the full-bleed photo with the logo, headline and subhead on it,
    ...
    <div class="w6wf-hero" style="position:relative;width:100%;max-width:600px;height:772px;overflow:hidden;clip-path:polygon(0 0,100% 0,100% calc(100% - 40px),74% calc(100% - 40px),68% 100%,0 100%);">
      ...
      <div style="position:absolute;top:0;left:0;right:0;bottom:0;background:linear-gradient(to bottom,rgba(0,0,0,0.55) 0%,rgba(0,0,0,0.25) 40%,rgba(0,0,0,0.45) 100%);">
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;border-collapse:collapse;">
          <tr><td valign="top" align="center" style="vertical-align:top;text-align:center;padding:${logoTop}px ${textLeft}px 0;line-height:normal;">
            ${logoOverlayOnHero}
            <div class="w6wf-herotext" style="text-align:center;margin-top:${textTop + 170}px;">
              ${copy.headlineText ? `<div class="w6wf-headline" style="font-family:'Lora',serif;font-size:${textSize}px;font-weight:700;color:#ffffff;line-height:1.12;text-shadow:0 2px 20px rgba(0,0,0,.3);">${copy.headlineText}</div>` : ''}
              ${subhead ? `<div style="font-family:Arial,sans-serif;font-size:18px;line-height:28px;color:#ffffff;text-shadow:0 1px 8px rgba(0,0,0,.35);margin-top:16px;">${subhead}</div>` : ''}
            </div>
          </td></tr>
        </table>
      </div>
  ...
  ${points.length ? `<div class="w6wf-section" style="padding:30px 48px 0;background-color:${pageBg};">
    ${copy.sectionHeadline ? `<div class="w6wf-h3" style="font-family:Arial,sans-serif;font-size:24px;line-height:32px;font-weight:700;text-transform:uppercase;color:${secondary};text-align:center;margin-bottom:6px;">${copy.sectionHeadline}</div>` : ''}
  ...
  ${(copy.bodyBlock2Title || policy) ? `<div class="w6wf-section" style="padding:30px 48px 0;background-color:${pageBg};">
    ${copy.bodyBlock2Title ? `<div class="w6wf-h3" style="font-family:Arial,sans-serif;font-size:24px;line-height:32px;font-weight:700;text-transform:uppercase;color:${secondary};">${copy.bodyBlock2Title}</div>` : ''}
  ...
  ${closing ? `<div class="w6wf-section" style="padding:16px 48px 0;background-color:${pageBg};">
    <div style="font-family:Arial,sans-serif;font-size:16px;line-height:24px;color:${mutedTextCol};">${closing}</div>
  </div>` : ''}
...
/* ─────────────────────────── Email 7 WF — midweek ───────────────────────────
   The one email in the flow whose headline is NOT on the photo. It opens the
   way the Airbnb reference does: the logo small and centred on the page, then
   the headline at full size beneath it, and only then the photo, full bleed.
   Nothing is baked onto the hero as a result — the photo is just a photo, so
   preview and inbox cannot drift the way they do on Emails 3 to 6.

   Two CTAs, the same button either side of the body. The pair of overlapping
   circles near the foot is Email 1's element, drawn live for the preview and
   baked flat once Generate Images has run.                                  */
function buildTemplateWeek7WF({ client, copy, images, footerData, isHeroGenerated = false,
  heroScale=1, heroX=0, heroY=0,
  textSize=38,
  ...
  /* One button, drawn twice. Baked PNG when Generate Images has run, otherwise
...
<table class="w7wf-outer" cellpadding="0" cellspacing="0" bgcolor="${pageBg}" style="width:100%;max-width:600px;margin:0 auto;background-color:${pageBg};border-collapse:collapse;border-radius:20px;overflow:hidden;">
<tr><td style="background-color:${pageBg};">

  <!-- MASTHEAD — logo, then the headline at full size. Both sit on the page,
       not on the photo, so both stay live text and neither is baked. -->
  <div class="w7wf-section" style="padding:${logoTop}px 48px 0;background-color:${pageBg};text-align:center;line-height:normal;">
    ${logoBlock}
  </div>
  ...
  ${subhead ? `<div class="w7wf-section" style="padding:26px 48px 0;background-color:${pageBg};">
    <div style="font-family:Arial,sans-serif;font-size:16px;line-height:24px;color:${mutedTextCol};">${subhead}</div>
  </div>` : ''}
  ${introCtaText ? `<div class="w7wf-section" style="padding:22px 48px 0;background-color:${pageBg};text-align:center;">
    ${ctaBtnHtml(introCtaText, introBtnImgUrl || btnImgUrl)}
  </div>` : ''}

  <!-- BODY -->
  ${introParas.length ? `<div class="w7wf-section" style="padding:26px 48px 0;background-color:${pageBg};">
  ...
  <!-- THE PAIR OF CIRCLES — Email 1's element. Sized in percentages against a
       600-wide stage, so the circles stay round at any width; once generated
       the whole thing arrives as one flat PNG. -->
...
/* ─────────────────────────── Email 9 WF — concierge close ───────────────────
   A plain, signed note that asks for a reply instead of a click. No hero and
   no headline: the logo on the page, the body, a framing line, then the
   example questions as Email 8's icon rows (the question in bold, the answer
   beneath), the close and the signature. A CTA and a code reminder render
   only when the copy carries them, which it usually does not. Email 1's pair
   of circles sits before the footer.                                        */
function buildTemplateWeek9WF({ client, copy, images, footerData,
  textSize=38,
  logoColor='original', logoTop=40, logoSize=44,
  img1Scale=1, img1X=0, img1Y=0,
  img2Scale=1, img2X=0, img2Y=0,
  btnImgUrl = null, iconImgUrls = [], gridImgUrl = null,
}) {
  const circleA = images?.[1]?.url || ''
  const circleB = images?.[2]?.url || ''
  const circleTfA = `translate(${img1X}px,${img1Y}px) scale(${img1Scale})`
  ...
  const signature  = (copy.signature || '').replace(/\n/g, '<br>')
  const footerLine = (copy.footerLine || '').replace(/\n/g, '<br>')
  const logoUrl    = client?.logoUrl || ''
  const introParas = String(copy.bodyText || '').split(/\n{2,}/).map(s => s.trim()).filter(Boolean)

  /* The questions as rows the icon picker understands. Five at most. */
  const points = (Array.isArray(copy.exampleQuestions) ? copy.exampleQuestions : [])
    .map(q => ({ title: (q?.question || '').trim(), text: (q?.answer || '').trim() }))
    .filter(q => q.title || q.text).slice(0, 5)
...
<table class="w9wf-outer" cellpadding="0" cellspacing="0" bgcolor="${pageBg}" style="width:100%;max-width:600px;margin:0 auto;background-color:${pageBg};border-collapse:collapse;border-radius:20px;overflow:hidden;">
<tr><td style="background-color:${pageBg};">

  <!-- MASTHEAD — the logo alone. A headline shows only if the copy has one. -->
  <div class="w9wf-section" style="padding:${logoTop}px 48px 0;background-color:${pageBg};text-align:center;line-height:normal;">
    ${logoBlock}
  </div>
  ${copy.headlineText ? `<div class="w9wf-section" style="padding:26px 48px 0;background-color:${pageBg};">
    <div class="w9wf-headline" style="font-family:Arial,sans-serif;font-size:${textSize}px;line-height:44px;font-weight:700;color:${textCol};letter-spacing:-0.01em;text-align:center;">${copy.headlineText}</div>
  </div>` : ''}

  <!-- BODY -->
  ${introParas.length ? `<div class="w9wf-section" style="padding:30px 48px 0;background-color:${pageBg};">
    ${introParas.map((para, i) => `<div style="font-family:Arial,sans-serif;font-size:16px;line-height:24px;color:${textCol};${i ? 'margin-top:14px;' : ''}">${para}</div>`).join('')}
  </div>` : ''}
  ${framing ? `<div class="w9wf-section" style="padding:22px 48px 0;background-color:${pageBg};">
    <div style="font-family:Arial,sans-serif;font-size:16px;line-height:24px;color:${textCol};">${framing}</div>
  </div>` : ''}

  <!-- THE QUESTIONS — a rule, one row per question: icon, the question in bold, the answer -->
  ${points.length ? `<div class="w9wf-section" style="padding:22px 48px 0;background-color:${pageBg};">
    <div style="height:1px;background:${cardBorder};font-size:0;line-height:0;"></div>
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;border-collapse:collapse;">
      ${points.map(pointRow).join('')}
    </table>
  ...
  <!-- THE CLOSE, then the signature -->
  ${closing ? `<div class="w9wf-section" style="padding:26px 48px 0;background-color:${pageBg};">
    <div style="font-family:Arial,sans-serif;font-size:16px;line-height:24px;color:${textCol};">${closing}</div>
  </div>` : ''}
  ${signature ? `<div class="w9wf-section" style="padding:18px 48px 0;background-color:${pageBg};">
    <div style="font-family:Arial,sans-serif;font-size:16px;line-height:24px;color:${mutedTextCol};">${signature}</div>
  </div>` : ''}
  ${ctaButton ? `<div class="w9wf-section" style="padding:26px 48px 0;background-color:${pageBg};text-align:center;">${ctaButton}</div>` : ''}

  <!-- THE PAIR OF CIRCLES — Email 1's element, before the footer -->
  ${(circleA || circleB) ? `<div style="padding:34px 0 0;background-color:${pageBg};line-height:0;font-size:0;text-align:center;">
    ${gridImgUrl
      ? `<img src="${gridImgUrl}" alt="" width="600" style="width:100%;max-width:600px;height:auto;display:block;margin:0 auto;border:0;outline:none;"/>`
      : `<div style="position:relative;width:100%;max-width:600px;aspect-ratio:600/360;margin:0 auto;">
...
<table width="552" cellpadding="0" cellspacing="0" border="0" style="width:552px;border-collapse:collapse;background:transparent;">
  ...
  <tr>
    ...
    /* Email 6's hero: logo, headline and subhead on the photo, centred, no
       pill. Same offsets as the on-screen hero so nothing shifts on Generate. */
    const week6wfSubhead = generatedCopy?.sectionSubhead || generatedCopy?.subhead || ''
    const week6wfHeroHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"/>
<link href="https://fonts.googleapis.com/css2?family=Lora:wght@700&display=swap" rel="stylesheet"/>
<style>*{margin:0;padding:0;box-sizing:border-box}body{width:600px;background:transparent;}</style>
</head><body>
<div style="width:600px;background:transparent;box-sizing:border-box;line-height:0;font-size:0;">
  <div style="position:relative;width:600px;height:772px;overflow:hidden;clip-path:polygon(0 0,100% 0,100% calc(100% - 40px),74% calc(100% - 40px),68% 100%,0 100%);">
    ...
    <div style="position:absolute;top:0;left:0;right:0;bottom:0;background:linear-gradient(to bottom,rgba(0,0,0,0.55) 0%,rgba(0,0,0,0.25) 40%,rgba(0,0,0,0.45) 100%);">
      <div style="position:absolute;top:${logoTop}px;left:0;right:0;text-align:center;line-height:normal;">${week1wfLogoHtml}</div>
      <div style="position:absolute;left:${textLeft}px;right:${textLeft}px;top:${logoTop + logoSize + textTop + 170}px;text-align:center;line-height:normal;">
        <div style="font-family:'Lora',serif;font-size:${textSize}px;font-weight:700;color:#fff;line-height:1.12;text-shadow:0 2px 20px rgba(0,0,0,.3);">${headline}</div>
        ${week6wfSubhead ? `<div style="font-family:Arial,sans-serif;font-size:18px;line-height:28px;color:#fff;text-shadow:0 1px 8px rgba(0,0,0,.35);margin-top:16px;">${week6wfSubhead}</div>` : ''}
      </div>
    </div>
  </div>
</div>
    ...
    const week6wfIconKeys = isWeek6WF && Array.isArray(generatedCopy?.points)
      ...
      : isWeek9WF && Array.isArray(generatedCopy?.exampleQuestions)
      ? wfPickIcons(generatedCopy.exampleQuestions.slice(0, 5).map(q => ({ title: q?.question || '', text: q?.answer || '' })))
      : []
...
<div style="position:relative;width:600px;height:340px;overflow:hidden;line-height:0;font-size:0;">
  ...
  <div style="position:absolute;left:105px;top:0;width:390px;height:340px;border-radius:14px;overflow:hidden;">
    ...
    /* Email 7's pair of circles — Email 1's geometry, with Email 7's inputs.
       600 x 360 and transparent, so the page shows through between them. */
    const week7wfCirclesHtml = (isWeek7WF && (img1Url || img2Url)) ? `<!DOCTYPE html><html><head><meta charset="UTF-8"/>
<style>*{margin:0;padding:0;box-sizing:border-box}body{width:600px;background:transparent;}</style>
</head><body>
...
<div style="position:relative;width:${w}px;height:${hgt}px;border-radius:${r}px;overflow:hidden;">
  <img src="${url}" style="position:absolute;top:0;left:0;width:${w}px;height:${hgt}px;object-fit:cover;display:block;transform:translate(${x}px,${y}px) scale(${sc});transform-origin:center center;"/>
    ...
    /* Email 4's card gallery: the three block photos fanned as on screen, the
       leading one first in `order`. 340x340, transparent around the fan. */
    const week4wfStackHtml = (order) => {
...
<div style="position:relative;width:340px;height:340px;background:transparent;">${layers}</div>
</body></html>`
    }
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '80px 0', background: dark ? '#111' : '#fff', fontFamily: 'Inter, sans-serif' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <button
                  ...
                  disabled={!(generatedCopy?.headlineText || tpl?.id === 39)}
                  style={{
                    padding: '10px 28px', borderRadius: 8, border: 'none',
                    cursor: (generatedCopy?.headlineText || tpl?.id === 39) ? 'pointer' : 'not-allowed',
                    background: (generatedCopy?.headlineText || tpl?.id === 39) ? accent : (dark ? '#2a2a2a' : '#e5e7eb'),
                    color: (generatedCopy?.headlineText || tpl?.id === 39) ? '#fff' : (dark ? '#555' : '#aaa'),
